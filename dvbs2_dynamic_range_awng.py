...
if __name__ == '__main__':
    ...
    for FREQUENCY_LEVEL_OFFSET in DVBS_S2_FREQUENCY_LEVEL_OFFSET:
        ...
        specan = Ektsfu(sfu_ip)
        specan.set_digitaltv_coding_symbolrate_dvbs2("30.000000e6")
        ...
        del net
        net = ekt_net.EktNetClient('192.168.1.24', 9999)
        net.send_data(json.dumps({"cmd": "set_symbol_rate_data", "symbol_rate": str(30000)}))
        time.sleep(1)
        del net
        ...
        try:
            res = iterate_to_find_threshold(sfu_ip, -60, -80)
            print "current_time:{}, frequency：{}，symbol_rate：{}，{}".format(
                datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), str(FREQUENCY_LEVEL_OFFSET[0]), str(30000), res)
