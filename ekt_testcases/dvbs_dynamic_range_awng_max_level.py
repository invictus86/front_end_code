            ...
            for FREQUENCY_LEVEL_OFFSET in DVBS_S2_FREQUENCY_LEVEL_OFFSET:
                ...
                if lock_state == "1":
                    ...
                    write_test_result("./../ekt_log/test_result_sfe.txt",
                                      ("dvbs_dynamic_range_awng_max_level: current_time:{}, coderate：{}, frequency：{} MHz，symbol_rate：{} Ksym/s，{}".format(
                ...
                try:
                    start_data_result = mosaic_algorithm(sfe_ip, str("%.2f" % ((-10) - FREQUENCY_LEVEL_OFFSET[1])),
                                                         "-10")
                    print "dvbs_dynamic_range_awng_max_level: current_time:{}, coderate：{}, frequency：{} MHz，symbol_rate：{} Ksym/s，马赛克检测结果：{}".format(
                        datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), code_rate_cn[0],
                        str(FREQUENCY_LEVEL_OFFSET[0]), str(SYMBOL_RATE[1]),
                        start_data_result.get("detect_mosic_result"))
                    write_test_result("./../ekt_log/test_result_sfe.txt",
                                      "dvbs_dynamic_range_awng_max_level: current_time:{}, coderate：{}, frequency：{} MHz，symbol_rate：{} Ksym/s，马赛克检测结果：{}".format(
                                          datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), code_rate_cn[0],
                                          str(FREQUENCY_LEVEL_OFFSET[0]), str(SYMBOL_RATE[1]),
                                          start_data_result.get("detect_mosic_result")) + "\n")
                except:
