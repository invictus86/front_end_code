...
from ekt_lib.ekt_utils import write_test_result, write_json_file, read_json_file, find_level_offset_by_frequency, \
    dvbc_2_channel_bandwidth_json_to_csv
...
PARAMETER_LIST = [
    [FREQUENCY_666, LEVEL_OFFSET_666, LEVEL_50_666, MODULATION_256QAM, SYMBOL_RATE_6952, None]
]

my_file = Path("../../ekt_json/dvbc_2_channel_bandwidth.json")
if my_file.exists():
    pass
else:
    dict_test_parame_result = {}
    dict_test_parame_result["test_parame_result"] = PARAMETER_LIST
    write_json_file("../../ekt_json/dvbc_2_channel_bandwidth.json", dict_test_parame_result)
...
if __name__ == '__main__':
    ...
    """
    load_dict = read_json_file("../../ekt_json/dvbc_2_channel_bandwidth.json")
    sfu_ip = "192.168.1.50"
    ...
    for LOCK_PARAMETER in load_dict.get("test_parame_result"):
        ...
        if lock_state == "1":
            ...
            write_json_file("../../ekt_json/dvbc_2_channel_bandwidth.json", load_dict)
            dvbc_2_channel_bandwidth_json_to_csv("../../ekt_json/dvbc_2_channel_bandwidth.json",
                                                 "../../ekt_test_report/dvbc_2_channel_bandwidth.csv")
            continue
        else:
        ...
        start_data_result, mosaic_result = mosaic_algorithm(sfu_ip, str("%.2f" % ((-50) - FREQUENCY_LEVEL_OFFSET[1])),
                                                            str("%.2f" % ((-50) - FREQUENCY_LEVEL_OFFSET[1])))
        print (
            "dvbc_2_channel_bandwidth: current_time:{}, frequency：{} MHz，symbol_rate：{} Ksym/s，level：{} dbm, 马赛克检测结果：{}".format(
                datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                str(FREQUENCY_LEVEL_OFFSET[0]), str(SYMBOL_RATE[1]),
                str("%.2f" % ((-50) - FREQUENCY_LEVEL_OFFSET[1])),
