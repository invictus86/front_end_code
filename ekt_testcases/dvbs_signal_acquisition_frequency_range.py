            ...
            for FREQUENCY_OFFSET in SYMBOL_RATE_FREQUENCY[2]:
                ...
                if lock_state == "1":
                    ...
                    write_test_result("./../ekt_log/test_result_sfe.txt",
                                      ("dvbs_signal_acquisition_frequency_range; current_time:{}, coderate：{}, frequency：{} MHz，symbol_rate：{} Ksym/s，{}".format(
                ...
                try:
                    ...
                    print "dvbs_signal_acquisition_frequency_range; current_time:{}, coderate：{}, frequency：{} MHz，symbol_rate：{} Ksym/s，马赛克检测结果：{}".format(
                        datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), code_rate_cn,
                        FREQUENCY_OFFSET[1], SYMBOL_RATE_FREQUENCY[1], start_data_result.get("detect_mosic_result"))
                    write_test_result("./../ekt_log/test_result_sfe.txt",
                                      "dvbs_signal_acquisition_frequency_range; current_time:{}, coderate：{}, frequency：{} MHz，symbol_rate：{} Ksym/s，马赛克检测结果：{}".format(
                                          datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), code_rate_cn,
