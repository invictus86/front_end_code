...
my_file = Path("../../ekt_json/dvbs_11_dynamic_range_awng_sensitivity_level.json")
if my_file.exists():
    pass
else:
    ...
    for SYMBOL_RATE in dict_config_data.get("SYMBOL_RATE"):
        for FREQUENCY_LEVEL_OFFSET in DVBS_S2_FREQUENCY_LEVEL_OFFSET:
            ...
            for code_rate_cn in DVBS_QPSK_CODE_RATE_CN:
                list_test_result.append([code_rate_cn, None, None, None])
            list_test_parame_result.append([SYMBOL_RATE, FREQUENCY_LEVEL_OFFSET, list_test_result])
    dict_test_parame_result["test_parame_result"] = list_test_parame_result

    write_json_file("../../ekt_json/dvbs_11_dynamic_range_awng_sensitivity_level.json", dict_test_parame_result)
...
if __name__ == '__main__':
    ...
    dvbs_dynamic_min_json_class_test("../../ekt_json/dvbs_11_dynamic_range_awng_sensitivity_level.json")
    load_dict = read_json_file("../../ekt_json/dvbs_11_dynamic_range_awng_sensitivity_level.json")
    sfe_ip = ekt_cfg.SFE_IP
    sfe_init_setting()
    ...
    for PARAMETER in load_dict.get("test_parame_result"):
        ...
        if lock_state == "1":
            ...
            write_test_result("../../ekt_log/test_result_sfe.txt",
                              (
                                      "dvbs_11_dynamic_range_awng_sensitivity_level: current_time:{}, frequency:{} MHz,symbol_rate:{} Ksym/s,{}".format(
        ...
        for code_rate_cn in PARAMETER[2]:
            ...
            write_json_file("../../ekt_json/dvbs_11_dynamic_range_awng_sensitivity_level.json", load_dict)
            write_test_result("../../ekt_log/test_result_sfe.txt",
                              "dvbs_11_dynamic_range_awng_sensitivity_level: current_time:{}, coderate:{}, frequency:{} MHz,symbol_rate:{} Ksym/s,level:{} dbm, Mosaic results:{}".format(
            ...
            res, test_result = iterate_to_find_threshold_step_by_step(sfe_ip,
                                                                      float((test_start_level) - FREQUENCY_LEVEL_OFFSET[1]),
                                                                      level_offset=str(FREQUENCY_LEVEL_OFFSET[1]))
            print (
                "dvbs_11_dynamic_range_awng_sensitivity_level: current_time:{}, coderate:{}, frequency:{} MHz,symbol_rate:{} Ksym/s,{}".format(
                    datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), code_rate_cn[0][0],
            ...
            if test_result is None:
                pass
            elif float(test_result) <= ekt_cfg.DVBS_11_SPEC:
                code_rate_cn[3] = "Pass"
            elif float(test_result) > ekt_cfg.DVBS_11_SPEC:
                code_rate_cn[3] = "Fail"
            else:
                code_rate_cn[3] = "test result err"
            write_json_file("../../ekt_json/dvbs_11_dynamic_range_awng_sensitivity_level.json", load_dict)
            dvbs_dynamic_min_json_to_csv("../../ekt_json/dvbs_11_dynamic_range_awng_sensitivity_level.json",
                                         "../../ekt_test_report/dvbs_11_dynamic_range_awng_sensitivity_level.csv")
