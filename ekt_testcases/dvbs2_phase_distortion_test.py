            ...
            for FREQUENCY_LEVEL_OFFSET in FREQUENCY_LEVEL_OFFSET_LIST:
                ...
                if lock_state == "1":
                    ...
                    write_test_result("../ekt_log/test_result_sfu.txt",
                                      ("dvbs2_phase_distortion_test: current_time:{}, coderate：{}, frequency：{} MHz，symbol_rate：{} Ksym/s，{}".format(
                ...
                try:
                    ...
                    print "dvbs2_phase_distortion_test: current_time:{}, coderate：{}, frequency：{} MHz，symbol_rate：{} Ksym/s，马赛克检测结果：{}".format(
                        datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), code_rate,
                        FREQUENCY_LEVEL_OFFSET[0], str(SYMBOL_RATE[1]), start_data_result.get("detect_mosic_result"))
                    write_test_result("../ekt_log/test_result_sfu.txt",
                                      "dvbs2_phase_distortion_test: current_time:{}, coderate：{}, frequency：{} MHz，symbol_rate：{} Ksym/s，马赛克检测结果：{}".format(
                                          datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), code_rate,
